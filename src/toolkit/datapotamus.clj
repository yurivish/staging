    ...
    (fn
      ...
      ([s in-id m]
       (cond
         ...
         ;; `tok/split-tokens`. This preserves the XOR invariant of any
         ;; upstream fan-out group across multi-port intermediaries
         ;; (routers, user-defined multi-out steps). Matches `emit`'s
         ;; split-across-ports semantics for data messages. For N=1
         ;; (single output) the split is identity — behavior unchanged
         ;; for ordinary single-`:out` chains. Terminal steps (no :outs)
         ;; absorb the signal.
         (let [_         (sp-pub step-sp (recv-signal-event trace-sid m))
               out-ports (keys outs)]
           (if (seq out-ports)
             (let [n        (count out-ports)
                   splits   (tok/split-tokens (:tokens m) n)
                   forwards (into {}
                                  (map-indexed (fn [i p]
                                                 [p [(-> (child-signal m)
                                                         (assoc :tokens (nth splits i)))]]))
                                  out-ports)]
  ...
  (let [pairs (vec (partition 2 port-data-pairs))
    ...
    (if (zero? n)
      ...
      (let [splits (tok/split-tokens (:tokens m) n)]
        (->> (map-indexed (fn [i [port data]]
                            [i port (-> (child-with-data m data)
                                        (assoc :tokens (nth splits i)))])
                          pairs)
             (reduce (fn [acc [_ port child]]
                       (update acc port (fnil conj []) child))
                     {}))))))
           ...
           (handler-factory
            (fn [_ctx s m]
              (let [gid        (str group-prefix (:msg-id m))
                    ...
                    ;; Split parent tokens N ways so every upstream
                    ;; zero-sum group's mass is conserved (not duplicated)
                    ;; across children. Parallel to what `emit` does.
             ...
             (let [routes  (vec (route-fn (:data m)))
                   ...
                   out-map (reduce (fn [acc i]
                                     (let [{:keys [data port]} (nth routes i)]
                                       ...
                                       (let [kid (-> (child-with-data m data)
