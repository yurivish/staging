...
Anthropic Claude and Google Gemini; one-shot and streaming; unstructured
and JSON-schema-structured output. Uses the `http-kit` client already in
`deps.edn`; no new dependency.
...
(def claude (anthropic/make {:api-key (System/getenv "ANTHROPIC_API_KEY")}))
(def gemini (google/make    {:api-key (System/getenv "GEMINI_API_KEY")}))

;; One-shot; returns {:text joined-string :raw parsed-response}
(llm/query-text claude
  {:model "claude-haiku-4-5"
   :max-tokens 64
   :messages [(llm/user-msg "Say hi in one word.")]})
;; => "Hi"

;; Structured output — pass a JSON schema as a Clojure map.
(llm/query-text claude
  {:model "claude-sonnet-4-6"
   :max-tokens 128
   :messages [(llm/user-msg "name & age of a fictional dog")]
...
;; Streaming — [ch stop!]. Errors arrive on the channel as Throwable.
(let [[ch stop!] (llm/query-stream gemini
                   {:model "gemini-2.5-flash"
                    :max-tokens 200
                    :messages [(llm/user-msg "Count to 5 slowly.")]})]
  (loop []
    (when-let [v (async/<!! ch)]
      (if (instance? Throwable v)
        (throw v)
        (do (print v) (flush) (recur)))))
  (stop!))
```

Fan-in multiple generations with `alts!!`:

```clojure
(let [streams (for [q ["one" "two" "three"]]
                (llm/query-stream claude
                  {:model "claude-haiku-4-5" :max-tokens 100
                   :messages [(llm/user-msg q)]}))
      chans   (mapv first streams)]
  (loop []
    (when (seq chans)
      (let [[v c] (async/alts!! chans)]
        (when v (print v) (flush))
        (recur)))))
```

Public API (`toolkit.llm`):
- `query` / `query-text` / `query-stream` — call through to the
  `Provider` protocol.
- `user-msg` / `assistant-msg` / `text-block` — small constructors for
  the common text-only message shape.
- `Provider` protocol — two methods, `-query` and `-query-stream`. Extend
  it directly to add a new provider without touching `toolkit.llm` itself.
- `start-sse-stream!` / `fail!` / `read-json` / `write-json` /
  `check-required!` — shared helpers used by the provider namespaces.

Providers:
- `toolkit.llm.anthropic/make {:api-key ...}` → `AnthropicProvider`.
- `toolkit.llm.google/make    {:api-key ...}` → `GoogleProvider`.
...
{:model         "claude-sonnet-4-6"    ; required
 ...
 :output-schema {:type "object" ...}}  ; optional; nil = unstructured
```

Content blocks support plain text and Anthropic-style documents
(`{:type "document" :source {...}}`) with `:base64`, `:text`, and
`:content` source types. Gemini translates these into its `inlineData`
and `parts` shape automatically.

`query-text` joins per-provider segments with the separator native to
that provider: Anthropic joins content blocks with `"\n"` (one block
per paragraph/tool-call); Gemini joins `candidates[].content.parts[]`
with `""` (parts are sub-token fragments, not logical units). If you
need a canonical form, post-process the `:raw` response from `query`.

Errors are `ex-info` with a `:reason` keyword:
- `:invalid-request` / `:invalid-config` — caller-side validation.
- `:http-status` — non-200 from the provider; carries `:status` and `:body`.
- `:http-error` — transport-level failure; carries `:cause`.
- `:parse-error` — response body wasn't valid JSON.
- `:max-tokens` — the stream hit the token cap.
- `:stream-error` — provider-reported stream error.

Streaming contract (`query-stream`):
- Returns `[ch stop!]`.
- `ch` receives strings as chunks arrive; closed when the stream ends.
- On error, a `Throwable` is put on `ch` and the channel is closed.
  Consumers should `(instance? Throwable v)` on each take.
- `stop!` is idempotent; aborts the HTTP stream and closes the channel.
  Always call `stop!` when you stop consuming early, or the reader
  thread leaks.

Intentionally out of scope for v1 (the Go counterpart at `/work/llm/`
has these; they're deferred):
- Response caching. Would slot in as a decorator over any `Provider`,
  backed by `toolkit.lmdb`.
- Per-model rate limiting. Token-bucket decorator.
- Request deduplication (singleflight). Decorator.
- Incremental JSON parsing of the streaming text (auto-close open
  strings / arrays / objects for partial-but-valid JSON). Will slot into
  the existing stream protocol as tagged events.
- JSON-schema builder helpers. For now callers hand in the JSON schema.
- Tool use, temperature / top-p, and multi-candidate sampling — add
  alongside `:max-tokens` when needed.
