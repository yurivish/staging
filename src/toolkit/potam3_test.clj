(ns toolkit.potam3-test
  "Narrative test suite for toolkit.potam3.
...
;; :on-signal broadcasts one signal-draft to every output. Tokens split K-ways
;; so XOR reconstructs.
...
;; `merge` produces a multi-parent derivation — the emitted draft has two
...
;; recipes live in potam3.md (they are ≤6-line patterns, not framework
;; features).
;; ============================================================================
...
;; parallelism is real, not cooperative. Per-worker trace scopes
;; (<id>.w0, <id>.w1, ...) make utilization visible in the event stream.
;; ============================================================================
...
;; [:flow "pool.w<i>"] segment), so we capture the scope vector rather than
;; the (still-shared) :step-id.
